...
This section presents the translation from Aeneas's Pure AST to Isabelle/HOL. We first describe the overall workflow and a predefined library, and then present the translation of types, expressions, mutable borrows, control flow, recursion, generics, and traits.

\subsection{Translation Overview}
\label{sec:workflow}

\autoref{fig:overview} presents the architecture of \texttt{Rust2Isabelle} within the Charon--Aeneas toolchain. Charon extracts LLBC from Rust, and Aeneas translates LLBC into a typed Pure AST together with the metadata required by subsequent extraction. These existing components are shown in grey. The Pure AST marks the boundary between Aeneas and the Isabelle/HOL backend.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/structure.png}
  \caption{Architecture of \texttt{Rust2Isabelle} within the Charon--Aeneas toolchain. Highlighted components show the Isabelle-specific extensions, and the dashed connection indicates dependency relation on the Prelude library.} 
  \label{fig:overview}
\end{figure}
\texttt{Rust2Isabelle} first constructs an Isabelle/HOL extraction context containing the Pure declarations, target names, built-in mappings, and dependency information.
The backend uses this context to translate each output module into an Isabelle/HOL theory.
Then, it emits the \texttt{theory} header and imports the prelude library, denoted by \rprelude{}, together with any user-specified theories.
Finally, it places the declarations in the dependency order supplied by Aeneas.

Pure constructs with direct Isabelle/HOL counterparts are translated into the corresponding types, terms, and declarations.
The backend represents Rust-specific features that require additional definitions using the predefined datatypes and built-in operations provided by the \rprelude{}, described in \autoref{sec:primitives}.

The backend uses \texttt{axiomatization} for external functions and globals that have only signatures, unsupported built-ins, generic trait methods, and mutually recursive groups not covered by the current termination rules.
It cannot yet represent recursive functions that may diverge.
The evaluation reports these assumptions separately from concrete definitions.


\subsection{Rust Primitives in Isabelle/HOL}
\label{sec:primitives}

Following Aeneas's backend design, we implement the \rprelude{} as a fixed Isabelle/HOL theory that provides target-side supports for the basic Rust primitives.
Recognized built-ins are mapped to definitions in the \rprelude{} or Isabelle/HOL. For example, Rust's \texttt{Option<T>} reuses Isabelle/HOL's \texttt{'a option} type and its \texttt{None} and \texttt{Some} constructors.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{@{}p{0.39\textwidth}p{0.54\textwidth}@{}}
    \toprule
    \textbf{Pure/Rust construct} & \textbf{Isabelle/HOL representation} \\
    \midrule
...
Rust type parameters such as \texttt{T} are translated to Isabelle/HOL type variables such as \texttt{'a}, making generated functions polymorphic.
...
\end{figure}
Each variant of the Rust enum becomes a constructor of the Isabelle/HOL datatype.
The recursive \texttt{Node} declaration is also translated into a datatype, since Isabelle/HOL records cannot be recursive. Its named constructor arguments retain the source field names.
...
Typed Pure expressions are translated recursively into Isabelle/HOL terms.
The translation follows the structure of the Pure AST and maps each expression form to its corresponding Isabelle/HOL syntax.~\autoref{tab:expression-map} summarizes the main translation rules.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{@{}p{0.42\textwidth}p{0.56\textwidth}@{}}
    \toprule
    \textbf{Pure expression} & \textbf{Isabelle/HOL term} \\
    \midrule
    Variable or literal
      & \texttt{x} or \texttt{c} \\
...
Most expression forms are translated structurally, with function and constructor arguments retaining their order in the Pure AST. Field access is a target-specific exception. Since Isabelle/HOL record selectors are functions, an access \texttt{r.f} is translated into \texttt{f r}. Record updates use Isabelle/HOL's form \texttt{r(\textbar{} f := e \textbar{})}. Parentheses are added when necessary to preserve the grouping of nested expressions.

Pure let-expressions are translated either as ordinary Isabelle/HOL
...
To avoid name collisions, the backend appends a trailing prime to Rust identifiers that conflict with Isabelle/HOL keywords or predefined constants.
...
The update therefore reaches exactly the selected owner.  The Isabelle/HOL backend preserves this functional representation directly, without introducing a target heap or a separate alias relation.

\subsection{Control flow and Recursion}
\label{sec:recursion}

Conditionals and pattern matching translate directly into Isabelle/HOL \texttt{if} and \texttt{case} expressions, as shown in \autoref{tab:expression-map}.
...
Termination is undecidable in general, but Isabelle/HOL can prove it for many common cases when supplied with a variant, namely a value in a well-founded order that strictly decreases on every recursive call or continuing loop iteration.
The backend currently derives suitable measures for structural recursion and selected integer-countdown patterns.
More general termination measures are not yet inferred.
...
The current Isabelle/HOL loop operator leaves such executions unspecified.
...
Non-recursive functions use Isabelle/HOL's \texttt{definition} command.
Recursive functions use \texttt{function}, while mutually recursive functions are emitted together as a joint function group.
For the currently supported structural and integer-countdown patterns, the backend supplies a well-founded measure and discharges the resulting termination obligations automatically.

\subsection{Generics and Traits}
\label{sec:traits}

Rust type parameters become polymorphic Isabelle/HOL type variables and require no runtime representation.
Const generics require a different treatment because Isabelle/HOL types cannot take const generics as type parameters.
We therefore translate a const-generic Rust structure into an Isabelle/HOL record, retaining \texttt{N} as a ghost field of type \texttt{usize}, while occurrences of \texttt{N} in function signatures become explicit \texttt{usize} parameters. \autoref{fig:const-generic-translation} illustrates this translation.
...
\end{minted}
\end{minipage}
\caption{A Rust trait, its implementation, and a trait-bounded generic function represented using an explicit Isabelle/HOL dictionary.}
...
dictionaries.  Trait clauses attached directly to type declarations are notyet supported.  Methods with their own type parameters cannot be stored
polymorphically in an Isabelle/HOL record and are currently handled using
top-level axiomatizations.
Trait objects are supported only for the restricted forms represented by the existing \texttt{Dyn} encoding.  More general \texttt{dyn Trait} combinations remain unsupported.
