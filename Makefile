...
ISABELLE_TEST_RUNNER_EXE ?= $(PWD)/bin/isabelle_test_runner
...
LLBC_DIR ?= tests/llbc
# The directory where the standalone Isabelle runner writes theory files.
ISABELLE_DEST_DIR ?= tests/isabelle
...
# Test the project by translating test files to various backends and running
# the Rust unit tests.
.PHONY: test
test: build-dev extract-tests cargo-test

.PHONY: test-isabelle
test-isabelle: build-dev extract-tests-isabelle cargo-test

# Translate every standalone Rust file and Cargo crate in tests/src to Isabelle.
# This deliberately ignores all per-test directives parsed by the regular runner.
.PHONY: isabelle
isabelle: build-dev
	$(ISABELLE_TEST_RUNNER_EXE) $(CHARON_EXE) $(AENEAS_EXE) $(LLBC_DIR) $(INPUTS_DIR) $(ISABELLE_DEST_DIR) $(AENEAS_OPTIONS)
...
# Isabelle input tests
ISABELLE_INPUTS_LIST := $(subst test-,test-isabelle-,$(INPUTS_LIST))

# Run all the tests we found.
.PHONY: extract-tests
extract-tests: $(INPUTS_LIST)

.PHONY: extract-tests-isabelle
extract-tests-isabelle: $(ISABELLE_INPUTS_LIST)
...
.PHONY: test-isabelle-%
test-isabelle-%: build-dev
	@echo "========================================"
	@echo "  TEST: $*"
	@echo "========================================"
	$(TEST_RUNNER_EXE) $(CHARON_EXE) $(AENEAS_EXE) $(LLBC_DIR) $(INPUTS_DIR)/"$*" isabelle $(AENEAS_OPTIONS)
	@echo "\033[32m[ISABELLE TEST]\033[0m $* done\n"

# Run the Rust unit tests (for test files with #[cfg(test)] modules).

# Single-file test crates with #[cfg(test)]
CARGO_TEST_FILES := $(shell grep -rl 'cfg(test)' $(INPUTS_DIR)/*.rs 2>/dev/null | grep -v '~$$')
# Directory test crates with Cargo.toml
CARGO_TEST_DIRS := $(shell find $(INPUTS_DIR) -mindepth 2 -name Cargo.toml -exec dirname {} \;)
