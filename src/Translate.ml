          ...
          *)
          let extract_decrease decl =
            ...
            if has_decr_clause then
              ...
              | Isabelle ->
                  raise
                    (Failure "Isabelle doesn't have template decreases/termination clauses")
          in
...
let extract_file (config : gen_config) (ctx : gen_ctx) (fi : extract_file_info)
  ...
  | Isabelle -> Printf.fprintf out "end\n");
  ...
  let namespace, crate_name, full_dest_dir, extract_filebasename =
    ...
    | Some filename ->
        ...
        let namespace =
          ...
          | None -> (
              match Config.backend () with
              | FStar | Coq | HOL4 | Isabelle -> crate.name
              | Lean -> crate.name)
        in
        let full_dest_dir =
          match subdir with
